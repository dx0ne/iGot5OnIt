...
function computeScore() {
    ...
    for (let row = 0; row < rows - 1; row++) { // Exclude the last row
        const rowScore = calculateLinePoints(gridSymbols[row]);
        score += rowScore;
        const rowElement = document.getElementById(`drag_${row}_${cols - 2}`).parentElement;
    ...
    for (let col = 0; col < cols - 1; col++) { // Exclude the last column
        const column = gridSymbols.map(row => row[col]);
        const colScore = calculateLinePoints(column);
        score += colScore;
        const colElement = document.getElementById(`drag_${rows - 2}_${col}`).parentElement;
